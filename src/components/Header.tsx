...
const Header: React.FC = () => {
  ...
  return (
    <header className="flex items-center justify-between p-2 sm:p-3 md:p-4 border-b shadow-sm">
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button 
              variant="ghost" 
              size="icon"
              className="rounded-full h-7 w-7 sm:h-8 sm:w-8 md:h-9 md:w-9 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors relative"
            >
              <BellRing className="h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5" />
              {recentNotifications.length > 0 && (
                <span className="absolute top-0 right-0 h-2 w-2 sm:h-2.5 sm:w-2.5 bg-red-500 rounded-full" />
              )}
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-64 p-0" align="end">
            ...
            <div className="max-h-64 overflow-auto">
              {recentNotifications.length === 0 ? (
                ...
                recentNotifications.map((transaction) => (
                  <div key={transaction.id} className="p-3 border-b last:border-0 hover:bg-muted/50">
        ...
        <Badge 
          ...
          className={`${isAdmin ? "bg-upi-blue text-white" : ""} text-xs sm:text-sm`}
        >
